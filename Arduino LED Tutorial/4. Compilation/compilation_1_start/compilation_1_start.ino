...
/*
   ...
   Add additional functions to trigger here
*/
void triggered() {
  clearLEDs();
...
void ringLoop() {
  ...
  int numberOfLoops = 1;
  int i = 0 ; // Current Loop
  while (i < numberOfLoops) {
    for (int i = 0; i < NUM_LEDS; i++) {
      leds1[i] = CUSTOM_COLOR;
      ...
      // For debugging
      totalDuration = delayDuration + totalDuration;
    }
...
void ringLoopWithAcceleration() {
  ...
  while ( delayDuration > 1) {
    ...
    delayDuration = delayDuration * 0.6;
  ...
  Serial.print("Time taken: ");
  Serial.println(totalDuration, 2);
  Serial.println("Exit ringLoopWithAcceleration()");
}
...
void breathingLight() {
  ...
  int d = 0;
  while (d < repeatTimes) {
    fill_solid(leds1, NUM_LEDS, CUSTOM_COLOR);
